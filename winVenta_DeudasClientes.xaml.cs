...
namespace sisgesoriadao
{
    ...
    public partial class winVenta_DeudasClientes : Window
    {
        ...
        {
            // Primera validación
            if (string.IsNullOrEmpty(txtPagoUSD.Text.Trim()) &&
                string.IsNullOrEmpty(txtPagoBOB.Text.Trim()))
            {
            ...
            if (double.Parse(txtPagoUSD.Text) == 0 ||
                double.Parse(txtPagoBOB.Text) == 0)
            {
                MessageBox.Show("No puede ingresar CERO como método de pago!.");
                return;
            }

            double pagoUSD = double.Parse(txtPagoUSD.Text.Trim());
            double pagoBOB = double.Parse(txtPagoBOB.Text.Trim());
            byte metodoPago = byte.Parse((cbxPaymentMethod.SelectedItem as ComboboxItem).Valor.ToString());

            // Tercera validación
            if (pagoUSD > SaldoUSD)
            {
                MessageBox.Show(
                    "ATENCIÓN, EL MONTO INGRESADO NO ES VÁLIDO PORQUE SUPERA EL SALDO TOTAL DE LA(S) VENTA(S)\n" +
                    "MONTO INGRESADO: " + pagoUSD + " $.\n" +
                    "SALDO TOTAL: " + SaldoUSD + " $.",
            ...
            // Confirmación
            if (MessageBox.Show(
                "Está a punto de saldar una o más ventas con saldo pendiente. ¿Está seguro de que desea continuar con el pago?",
                "REGISTRAR PAGOS Y ACTUALIZAR VENTAS",
            ...
            while (pagoUSD > 0 && i < listaIDVentas.Count)
            {
                double saldoVenta = listaSaldosUSD[i];
                double montoAPagar;

                if (pagoUSD >= saldoVenta)
                {
                    montoAPagar = saldoVenta; // pago total
                }
                else
                {
                    montoAPagar = pagoUSD; // pago parcial
                }

                string insert = implVenta.InsertPaymentMethodTransaction(
                    listaIDVentas[i],
                    montoAPagar,
                    Math.Round(montoAPagar * Session.Ajuste_Cambio_Dolar, 2),
                    metodoPago
                );

                if (insert == "INSERTMETODOPAGO_EXITOSO")
                {
                    mensaje += montoAPagar == saldoVenta
                        ? "¡Pago EXITOSO en la venta #" + listaIDVentas[i] + "!\n"
                        : "Se pagó PARCIALMENTE la venta #" + listaIDVentas[i] + "\n";
                }
                else
                {
                    mensaje += insert + "\n";
