...
namespace sisgesoriadao.Implementation
{
    public class ProductoImpl : DataBase, IProducto
    {
        ...
        {
            string query = @"SELECT P.idProducto AS ID, S.nombreSucursal AS Sucursal, C.nombreCategoria AS Categoria, CC.nombreCondicion AS Condicion, P.codigoSublote AS Codigo, P.nombreProducto AS Producto, P.identificador AS 'Identificador', P.costoUSD AS 'C USD', P.costoBOB AS 'C Bs', P.precioVentaUSD AS 'P USD', P.precioVentaBOB AS 'P BOB', P.observaciones AS Observaciones," + Session.FormatoFechaMySql("P.fechaRegistro") + @" AS 'Fecha de Registro', IFNULL(" + Session.FormatoFechaMySql("P.fechaActualizacion") + @",'-') AS 'Fecha de Actualizacion' FROM Producto AS P
        ...
        {
            string query = @"SELECT L.idLote AS ID, U.nombreUsuario AS Usuario, L.codigoLote AS 'Codigo', " + Session.FormatoFechaMySql("L.fechaRegistro") + @" AS 'Fecha de Registro', IFNULL(" + Session.FormatoFechaMySql("L.fechaActualizacion") + @",'-') AS 'Fecha de Actualizacion' FROM Lote AS L
        ...
        public DataTable SelectLikeInventoryFilter(string cadenaBusqueda, string idSucursales, string idCondiciones, string idCategorias, string estados)
        {
            string query = @"SELECT P.idProducto AS ID, S.nombreSucursal AS Sucursal, C.nombreCategoria AS Categoria, CC.nombreCondicion AS Condicion, P.codigoSublote AS Codigo, P.nombreProducto AS Producto, P.identificador AS 'Identificador', P.observaciones AS Observaciones, IF(P.estado=0,'Eliminado',IF(P.estado=1,'Disponible','Vendido')) AS Disponibilidad, P.precioVentaUSD AS PrecioUSD FROM Producto AS P
                                ...
                                AND (P.nombreProducto LIKE @search OR P.codigoSublote LIKE @search OR P.identificador LIKE @search)
                                ORDER BY 2 ASC, 6 ASC, 5 ASC";
            MySqlCommand command = CreateBasicCommand(query);
            command.Parameters.AddWithValue("@search", "%" + cadenaBusqueda + "%");
            try
        ...
        public DataTable SelectBatchOfProductsToUpdate(string CadenaBusqueda)
        {
            string query = @"SELECT idProducto, idCategoria, idCondicion, idSublote, codigoSublote AS Codigo, nombreProducto AS Producto,
                            ...
                            FROM Producto WHERE idSublote = (SELECT idSublote FROM Producto WHERE codigoSublote LIKE @search OR identificador LIKE @search LIMIT 1)
